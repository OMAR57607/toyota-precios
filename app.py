...
import urllib.parse
import math
import zipfile  # <--- AGREGADO: Necesario para leer el Excel dentro del ZIP
...
def cargar_catalogo():
    ...
    try:
        # 2. Abrir ZIP y buscar .xlsx
        with zipfile.ZipFile(archivo_zip, "r") as z:
            archivos_dentro = [f for f in z.namelist() if f.endswith('.xlsx')]
            
            if not archivos_dentro:
                # Si no hay excel, intentamos csv por si acaso, pero la prioridad es xlsx
                return None, None, None
        ...
        c_sku = next((c for c in df.columns if 'PART' in c or 'NUM' in c or 'ITEM' in c), None)
        c_desc = next((c for c in df.columns if 'DESC' in c), None)
        # Buscamos precio en varias variantes
        c_precio = next((c for c in df.columns if 'PRICE' in c or 'PRECIO' in c or 'TOTAL' in c or 'UNITARIO' in c), None)
        ...
        # 6. Procesar Precio (Crear columna PRECIO_NUM)
        ...
        # En producción podrías usar st.error(e) para depurar, pero para que no rompa la app devolvemos None
        return None, None, None
...
# 5. UI PRINCIPAL
# ==========================================
if df_db is None: st.error("Falta base_datos_2026.zip o el archivo no contiene un Excel válido."); st.stop()
